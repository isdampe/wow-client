...
class wowBattleground {
  ...
  main() {
    ...
    switch ( this.status ) {
      ...
      case BG_STATUS_GRAVEYARD:
        ...
        this.control.activateWindow();
        this.beginPathSet( this.findClosestGraveyardWaypoints(), () => {
  ...
  findClosestGraveyardWaypoints() {

    //Loop through each set of graveyard waypoints
    //Find closest distance of first waypoint in set
    ...
    for ( var i=0; i<this.waypoints.graveyard.length; i++ ) {
      let wp = this.waypoints.graveyard[i];
      let d = this.controller.calculateDistance( p.playerY, p.playerX, wp[0].y, wp[0].x );
    ...
    return this.waypoints.graveyard[closestIndex];
